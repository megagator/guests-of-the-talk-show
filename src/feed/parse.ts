...
import { SortOrder } from './enum/SortOrder.js'
import slugify from '../utilities/slugify.js'

fetch('https://daringfireball.net/thetalkshow/rss').then((response) => {
  response.text().then((xmlString) => {
    ...
    const titleRegex = /^(\w+):\s‘(.*)’,? with (?:(?:special )?guests? )?(.*)/i
    ...
    for (const podcast of js.rss.channel.item) {
      ...
      if (match) {
        ...
        const guestlessMatch = podcast.title._text.match(titleWithOutGuestRegex)
        if (guestlessMatch) {
    ...
    // all episodes
    fs.writeFileSync('content/episodes.json', JSON.stringify(podcasts, null, 2))
